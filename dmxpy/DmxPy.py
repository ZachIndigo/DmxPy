...
class DmxPy:
    def __init__(self, serial_port=None, baud_rate=57600, port_grep='0403:6001', channel_data='00', mod_type='set', dmx_size=64, path=''):
        self.dmx_size = dmx_size
        self.dmx_cache_fname = os.path.join(tempfile.gettempdir(), 'dmx_cache.txt')
...
def main():
    ...
    parser.add_argument('-c', '--channel_data', type=str, default='00',
                        help='channel data sent as pairs of hex, ie FFFF would be max level for channel 1 and 2')
    parser.add_argument('-t', '--mod-type', type=str, default='set', help='Specify whether channel data is set, up or down (default: set)')
    parser.add_argument('-s', '--size', type=int, default=64, help='DMX Size (default: 64)')
    parser.add_argument('-f', '--path', type=str, default='', help='File to load DMX channel presets')
    parser.add_argument('-b', '--blackout', action='store_true', help='Turn off all channels (level=0)')

    args = parser.parse_args()
    dmx = DmxPy(args.port, baud_rate=args.rate, port_grep=args.port_grep, channel_data=args.channel_data, mod_type=args.mod_type, dmx_size=args.size, path=args.path)
